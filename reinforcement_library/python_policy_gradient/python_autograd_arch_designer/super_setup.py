...
class Setup:
    ...
    #### run optimization ####
    def fit(self,x,y,beta,max_its,alpha,**kwargs):
        ...
        if algo == 'sgd':
            w_hist,cost_hist = super_optimizers.gradient_descent(self.cost,self.w_init,x,y,beta,alpha,max_its,self.batch_size,verbose=verbose)
        ...
        if algo == 'RMSprop':
            ...
            w_hist,cost_hist,self.avg_sq_grad = super_optimizers.RMSprop(self.cost,self.w_init,x,y,beta,alpha,max_its,self.batch_size,verbose=verbose,avg_sq_grad=self.avg_sq_grad)
